...
function Medical({ p }) {
  ...
  return (
    <div className="grid fade-in" style={{ gridTemplateColumns:'1fr 1fr' }}>
      <div className="card card-pad">
        <h3 style={{ fontSize:16, marginBottom:16 }}>Registration & contract</h3>
        <div style={{ display:'flex', flexDirection:'column', gap:0 }}>
          {[['BFF Player ID', 'BD-'+p.id.toUpperCase()+'-'+(2400+p.hue)],['Registration', 'Active · verified'],['Eligibility','Bangladesh (FIFA cleared)'],['Club contract', p.club],['Contract until', `Jun 30, ${2026+(p.hue%3)+1}`],['Estimated value', p.value]].map(([k,v],i)=>(
            <div key={k} className="row" style={{ justifyContent:'space-between', padding:'11px 0', borderBottom:i<5?'1px solid var(--line)':'none', fontSize:14 }}>
              <span style={{ color:'var(--ink-3)' }}>{k}</span>
              <b className={k==='BFF Player ID'?'num':''}>{v} {k==='Registration' && <Icon name="check" size={13} color="var(--pos)" style={{ verticalAlign:-2 }} />}</b>
            </div>
          ))}
        </div>
      </div>
...
function PlayerManagement({ go }) {
  ...
  return (
    <div className="content-inner fade-in">
      ...
      <div className="grid" style={{ gridTemplateColumns:'repeat(4,1fr)', marginBottom:'var(--gap)' }}>
        <Stat k="Total registered" v={D.allPlayers.length} d="national pool" dColor="var(--ink-3)" glyph="users" onClick={()=>{ setClub('All'); setQ(''); }} />
        <Stat k="Active" v={rows.filter(r=>r.reg==='Active').length} d="cleared to play" glyph="check" />
        <Stat k="Pending review" v={D.allPlayers.filter(p=>p.hue%7===0).length} d="needs action" dColor="var(--warn)" glyph="clock" accent="var(--warn)" />
        <Stat k="Expiring soon" v={D.allPlayers.filter(p=>p.hue%11===0).length} d="within 60 days" dColor="var(--neg)" glyph="bell" accent="var(--bff-red)" />
      </div>
